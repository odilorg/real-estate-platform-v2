...
export class TasksService {
  ...
  async create(developerId: string, dto: CreateTaskDto) {
    ...
    if (dto.leadId) {
      const lead = await this.prisma.developerLead.findFirst({
        where: { id: dto.leadId, developerId },
      });
      if (!lead) {
    ...
    if (dto.dealId) {
      const deal = await this.prisma.developerDeal.findFirst({
        where: { id: dto.dealId, developerId },
      });
      if (!deal) {
    ...
    return this.prisma.developerTask.create({
      data: {
        developerId,
        title: dto.title,
  ...
  async findAll(developerId: string, query: QueryTasksDto) {
    ...
    const [tasks, total] = await Promise.all([
      this.prisma.developerTask.findMany({
        where,
        skip,
        take,
        orderBy: [
      ...
      this.prisma.developerTask.count({ where }),
    ]);

    return { tasks, total, skip, take };
  }

  async findOne(developerId: string, id: string) {
    const task = await this.prisma.developerTask.findUnique({
      where: { id },
      include: {
        assignedTo: {
          select: {
  ...
  async update(developerId: string, id: string, dto: UpdateTaskDto) {
    const task = await this.findOne(developerId, id);
    ...
    return this.prisma.developerTask.update({
      where: { id },
      data: updateData,
      include: {
        assignedTo: {
  ...
  async remove(developerId: string, id: string) {
    await this.findOne(developerId, id); // Verify exists and belongs to agency
  ...
  async getStats(developerId: string) {
    ...
    const [totalTasks, pendingTasks, inProgressTasks, completedTasks, overdueTasks] = await Promise.all([
      this.prisma.developerTask.count({ where: { developerId } }),
      this.prisma.developerTask.count({ where: { developerId, status: 'PENDING' } }),
      this.prisma.developerTask.count({ where: { developerId, status: 'IN_PROGRESS' } }),
      this.prisma.developerTask.count({ where: { developerId, status: 'COMPLETED' } }),
      this.prisma.developerTask.count({
        where: {
          developerId,
