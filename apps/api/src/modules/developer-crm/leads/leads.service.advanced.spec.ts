...
Jane,Smith,+998902222222,jane@example.com`;
      ...
      prisma.developerLead.findFirst.mockResolvedValue(null); // No duplicates
      prisma.developerLead.create.mockResolvedValueOnce(
      ...
      const result = await service.importFromCSV(developerId, importDto);
...
Jane,Smith,+998901111111`;
      ...
      prisma.developerLead.findFirst.mockResolvedValueOnce(null);
      prisma.developerLead.findFirst.mockResolvedValueOnce(
        createMockLead({ phone: '+998901111111' }),
      );
      prisma.developerLead.create.mockResolvedValue(
...
John,Updated,+998901111111,updated@example.com`;
      ...
      prisma.developerLead.findFirst.mockResolvedValue(existingLead);
      prisma.developerLead.update.mockResolvedValue({
        ...existingLead,
        firstName: 'John',
        lastName: 'Updated',
      ...
      expect(result.skipped).toBe(0);
      expect(result.failed).toBe(0);
      expect(prisma.developerLead.update).toHaveBeenCalled();
    });
...
John,Doe,+998901111111`;
      ...
      prisma.developerLead.findFirst.mockResolvedValue(
        createMockLead({ phone: '+998901111111' }),
      );
...
John,Doe,+998901111111`;
      ...
      await service.importFromCSV(developerId, importDto);
...
John,Doe,+998901111111,APARTMENT,100000,2,"Юнусабад,Мирабад"`;
      ...
      prisma.developerLead.findFirst.mockResolvedValue(null);
      prisma.developerLead.create.mockResolvedValue(createMockLead());
    ...
    it('should handle CSV parsing errors', async () => {
      ...
      await expect(service.importFromCSV(developerId, importDto)).rejects.toThrow(
...
John,Doe,+998901111111,WEBSITE`;
  ...
  describe('exportToCSV', () => {
    ...
    it('should export leads to CSV format', async () => {
      ...
      prisma.developerLead.count.mockResolvedValue(2);

      const result = await service.exportToCSV(developerId, {});
    ...
    it('should include all lead fields in export', async () => {
      ...
      prisma.developerLead.findMany.mockResolvedValue([mockLead]);
      prisma.developerLead.count.mockResolvedValue(1);
    ...
    it('should apply query filters before exporting', async () => {
      ...
      await service.exportToCSV(developerId, queryDto);
  ...
  describe('bulkDelete', () => {
    ...
    it('should delete multiple leads successfully', async () => {
      ...
      prisma.developerLead.findUnique.mockImplementation(({ where }) => {
        return Promise.resolve(createMockLead({ id: where.id, developerId }));
      });
      prisma.developerLead.delete.mockResolvedValue(createMockLead());

      const result = await service.bulkDelete(developerId, bulkDeleteDto);

      expect(result.success).toBe(3);
      expect(result.failed).toBe(0);
      expect(result.errors).toHaveLength(0);
      expect(prisma.developerLead.delete).toHaveBeenCalledTimes(3);
    });
    ...
    it('should handle non-existent leads', async () => {
      ...
      prisma.developerLead.findUnique.mockImplementation(({ where }) => {
        if (where.id === 'lead-1') {
          return Promise.resolve(createMockLead({ id: 'lead-1', developerId }));
        }
    ...
    it('should prevent deleting leads from different agency', async () => {
      ...
      prisma.developerLead.findUnique.mockImplementation(({ where }) => {
        ...
        return Promise.resolve(
          createMockLead({ id: 'lead-2', developerId: 'different-agency' }),
        );
      });
      prisma.developerLead.delete.mockResolvedValue(createMockLead());
    ...
    it('should handle database errors gracefully', async () => {
      ...
      prisma.developerLead.findUnique.mockResolvedValue(
        createMockLead({ id: 'lead-1', developerId }),
      );
      prisma.developerLead.delete.mockRejectedValue(new Error('Database error'));
  ...
  describe('bulkAssign', () => {
    ...
    it('should assign multiple leads successfully', async () => {
      ...
      prisma.developerLead.update.mockResolvedValue(createMockLead());

      const result = await service.bulkAssign(developerId, bulkAssignDto);

      expect(result.success).toBe(3);
      expect(result.failed).toBe(0);
      expect(result.errors).toHaveLength(0);
      expect(prisma.developerLead.update).toHaveBeenCalledTimes(3);
    });
    ...
    it('should throw NotFoundException if member does not exist', async () => {
      ...
      await expect(service.bulkAssign(developerId, bulkAssignDto)).rejects.toThrow(
        NotFoundException,
      );
      await expect(service.bulkAssign(developerId, bulkAssignDto)).rejects.toThrow(
    ...
    it('should throw NotFoundException if member is not active', async () => {
      ...
      prisma.developerMember.findFirst.mockResolvedValue(null); // isActive: false filtered out
    ...
    it('should handle non-existent leads', async () => {
      ...
      prisma.developerMember.findFirst.mockResolvedValue(
        createMockMember({ id: memberId, developerId }),
      );
      prisma.developerLead.findUnique.mockImplementation(({ where }) => {
        if (where.id === 'lead-1') {
          return Promise.resolve(createMockLead({ id: 'lead-1', developerId }));
    ...
    it('should set assignedAt timestamp when assigning', async () => {
      ...
      await service.bulkAssign(developerId, bulkAssignDto);
