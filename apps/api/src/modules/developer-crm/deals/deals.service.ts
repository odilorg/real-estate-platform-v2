...
export class DealsService {
  ...
  async create(developerId: string, memberId: string, createDealDto: CreateDealDto): Promise<any> {
    // Verify lead exists and belongs to agency
    const lead = await this.prisma.developerLead.findFirst({
      where: { id: createDealDto.leadId, developerId },
    });
    ...
    const existingDeal = await this.prisma.developerDeal.findUnique({
    ...
    const owner = await this.prisma.developerMember.findFirst({
      where: { id: createDealDto.ownerId, developerId, isActive: true },
    });
    ...
    let commissionRate: number = createDealDto.commissionRate ?? 3.0;
    if (!createDealDto.commissionRate) {
      const developerCRM = await this.prisma.developerCRM.findUnique({
        where: { developerId },
      });
      commissionRate = developerCRM?.defaultCommissionRate || 3.0;
    }
    ...
    // Create deal
    const deal = await this.prisma.developerDeal.create({
      data: {
        ...createDealDto,
        developerId,
        commissionRate,
  ...
  async findAll(developerId: string, memberId: string, role: string, query: any): Promise<any> {
    ...
    const [deals, total] = await Promise.all([
      this.prisma.developerDeal.findMany({
        where,
        include: {
          lead: {
            select: {
      ...
      this.prisma.developerDeal.count({ where }),
    ]);
  ...
  async findOne(developerId: string, memberId: string, role: string, id: string): Promise<any> {
    const deal = await this.prisma.developerDeal.findFirst({
  ...
  async update(developerId: string, memberId: string, role: string, id: string, updateDealDto: UpdateDealDto): Promise<any> {
    const deal = await this.prisma.developerDeal.findFirst({
      where: { id, developerId },
    });
    ...
    const updatedDeal = await this.prisma.developerDeal.update({
      where: { id },
      data: updateData,
      include: {
        lead: true,
  ...
  async close(developerId: string, memberId: string, role: string, id: string, closeDealDto: CloseDealDto): Promise<any> {
    const deal = await this.prisma.developerDeal.findFirst({
      where: { id, developerId },
      include: {
        lead: true,
    ...
    // Update deal
    const closedDeal = await this.prisma.developerDeal.update({
      where: { id },
      data: {
    ...
    if (closeDealDto.status === 'WON') {
      await this.createCommission(developerId, deal);
    }
  ...
  async delete(developerId: string, _memberId: string, role: string, id: string): Promise<any> {
    const deal = await this.prisma.developerDeal.findFirst({
      where: { id, developerId },
    });
    ...
    await this.prisma.developerDeal.delete({
      where: { id },
    });
  ...
  async getPipeline(developerId: string, memberId: string, role: string): Promise<any> {
    const where: any = { developerId, status: 'ACTIVE' };
  ...
  private async createCommission(developerId: string, deal: any): Promise<void> {
    // Get agency fee percentage
    const developerCRM = await this.prisma.developerCRM.findUnique({
      where: { developerId },
    });

    const agencyFeePercent = developerCRM?.agencyFeePercent || 0;
    ...
    if (deal.commissionSplit && Array.isArray(deal.commissionSplit)) {
      ...
      for (const split of deal.commissionSplit) {
        ...
        await this.prisma.developerCommission.create({
          data: {
            developerId,
            dealId: deal.id,
