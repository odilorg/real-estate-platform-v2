...
export class NotificationsService {
  ...
  async create(data: {
    ...
    return this.prisma.developerNotification.create({
      data,
    });
  }
  ...
  async findAllForMember(
    ...
    return this.prisma.developerNotification.findMany({
      where,
      include: {
        task: {
          select: {
   ...
   */
  async getUnreadCount(developerId: string, memberId: string): Promise<number> {
    return this.prisma.developerNotification.count({
      where: {
        developerId,
        recipientId: memberId,
        isRead: false,
  ...
  async markAsRead(
    ...
    const notification = await this.prisma.developerNotification.findFirst({
      where: {
        id: notificationId,
        developerId,
        recipientId: memberId,
    ...
    return this.prisma.developerNotification.update({
   ...
   */
  async markAllAsRead(developerId: string, memberId: string): Promise<any> {
    return this.prisma.developerNotification.updateMany({
      where: {
        developerId,
        recipientId: memberId,
        isRead: false,
  ...
  async delete(
    ...
    await this.prisma.developerNotification.delete({
      where: { id: notificationId },
    });
  }
   ...
   */
  async notifyOverdueTasks(developerId: string): Promise<number> {
    const now = new Date();

    // Find tasks that are overdue and haven't sent a reminder yet
    const overdueTasks = await this.prisma.developerTask.findMany({
      where: {
        developerId,
   ...
   */
  async notifyTasksDueSoon(developerId: string): Promise<number> {
    const now = new Date();
    const tomorrow = new Date();
    tomorrow.setHours(tomorrow.getHours() + 24);

    const tasksDueSoon = await this.prisma.developerTask.findMany({
      where: {
        developerId,
        status: { in: ['PENDING', 'IN_PROGRESS'] },
        dueDate: {
