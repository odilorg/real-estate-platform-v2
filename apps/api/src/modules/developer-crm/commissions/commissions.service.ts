...
export class CommissionsService {
  ...
  async findAll(developerId: string, memberId: string, role: string, query: any): Promise<any> {
    ...
    const [commissions, total] = await Promise.all([
      this.prisma.developerCommission.findMany({
        where,
        include: {
          member: {
            include: {
      ...
      this.prisma.developerCommission.count({ where }),
    ]);
  ...
  async findOne(developerId: string, memberId: string, role: string, id: string): Promise<any> {
    const commission = await this.prisma.developerCommission.findFirst({
  ...
  async approve(developerId: string, role: string, id: string): Promise<any> {
    ...
    return this.prisma.developerCommission.update({
      where: { id },
      data: {
        status: 'APPROVED',
      },
  ...
  async markAsPaid(developerId: string, role: string, id: string, paymentData: any): Promise<any> {
  ...
  async getSummary(developerId: string, memberId: string, role: string, query: any): Promise<any> {
    const where: any = { developerId };
