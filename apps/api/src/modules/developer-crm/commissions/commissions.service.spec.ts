...
describe('CommissionsService', () => {
  ...
  describe('findAll', () => {
    ...
    it('should return paginated commissions for admin', async () => {
      ...
      prisma.developerCommission.findMany.mockResolvedValue(mockCommissions);
      prisma.developerCommission.count.mockResolvedValue(50);

      const result = await service.findAll(developerId, memberId, 'ADMIN', {});

      expect(result.data).toEqual(mockCommissions);
      expect(result.meta.total).toBe(50);
      expect(prisma.developerCommission.findMany).toHaveBeenCalled();
    });

    it('should filter commissions by status', async () => {
      prisma.developerCommission.findMany.mockResolvedValue([]);
      prisma.developerCommission.count.mockResolvedValue(0);

      await service.findAll(developerId, memberId, 'ADMIN', { status: 'PAID' });

      expect(prisma.developerCommission.findMany).toHaveBeenCalledWith(
    ...
    it('should allow admins to filter by specific member', async () => {
      ...
      await service.findAll(developerId, memberId, 'ADMIN', { memberId: targetMemberId });
    ...
    it('should support pagination', async () => {
      ...
      await service.findAll(developerId, memberId, 'ADMIN', { page: 2, limit: 10 });
  ...
  describe('findOne', () => {
    ...
    it('should return commission by id', async () => {
      ...
      prisma.developerCommission.findFirst.mockResolvedValue(mockCommission);

      const result = await service.findOne(developerId, memberId, 'ADMIN', commissionId);

      expect(result).toEqual(mockCommission);
      expect(prisma.developerCommission.findFirst).toHaveBeenCalledWith({
        where: { id: commissionId, developerId },
        include: expect.any(Object),
      });
    });

    it('should throw NotFoundException if commission not found', async () => {
      prisma.developerCommission.findFirst.mockResolvedValue(null);

      await expect(
        service.findOne(developerId, memberId, 'ADMIN', commissionId),
    ...
    it('should allow agent to view their own commission', async () => {
      ...
      const result = await service.findOne(developerId, memberId, 'AGENT', commissionId);
    ...
    it('should throw ForbiddenException if agent tries to view another member commission', async () => {
      ...
      await expect(
        service.findOne(developerId, memberId, 'AGENT', commissionId),
  ...
  describe('approve', () => {
    ...
    it('should approve commission successfully', async () => {
      ...
      prisma.developerCommission.findFirst.mockResolvedValue(mockCommission);
      prisma.developerCommission.update.mockResolvedValue(approvedCommission);

      const result = await service.approve(developerId, 'ADMIN', commissionId);

      expect(result).toEqual(approvedCommission);
      expect(prisma.developerCommission.update).toHaveBeenCalledWith({
    ...
    it('should throw ForbiddenException if agent tries to approve', async () => {
      await expect(service.approve(developerId, 'AGENT', commissionId)).rejects.toThrow(
        ForbiddenException,
      );
      await expect(service.approve(developerId, 'AGENT', commissionId)).rejects.toThrow(
    ...
    it('should throw NotFoundException if commission not found', async () => {
      ...
      await expect(service.approve(developerId, 'ADMIN', commissionId)).rejects.toThrow(
        NotFoundException,
      );
      await expect(service.approve(developerId, 'ADMIN', commissionId)).rejects.toThrow(
  ...
  describe('markAsPaid', () => {
    ...
    it('should mark commission as paid successfully', async () => {
      ...
      prisma.developerCommission.findFirst.mockResolvedValue(mockCommission);
      prisma.developerCommission.update.mockResolvedValue(paidCommission);

      const result = await service.markAsPaid(developerId, 'ADMIN', commissionId, paymentData);
    ...
    it('should throw ForbiddenException if agent tries to mark as paid', async () => {
      await expect(
        service.markAsPaid(developerId, 'AGENT', commissionId, paymentData),
      ).rejects.toThrow(ForbiddenException);
      await expect(
        service.markAsPaid(developerId, 'AGENT', commissionId, paymentData),
    ...
    it('should throw NotFoundException if commission not found', async () => {
      ...
      await expect(
        service.markAsPaid(developerId, 'ADMIN', commissionId, paymentData),
      ).rejects.toThrow(NotFoundException);
      await expect(
        service.markAsPaid(developerId, 'ADMIN', commissionId, paymentData),
      ).rejects.toThrow('Commission not found');
  ...
  describe('getSummary', () => {
    ...
    it('should return commission summary', async () => {
      ...
      prisma.developerCommission.findMany.mockResolvedValue(mockCommissions);
      prisma.developerCommission.aggregate.mockResolvedValue(mockAggregateResult);

      const result = await service.getSummary(developerId, memberId, 'ADMIN', {});
    ...
    it('should restrict agents to their own summary', async () => {
      ...
      await service.getSummary(developerId, memberId, 'AGENT', {});
    ...
    it('should support date range filtering', async () => {
      ...
      await service.getSummary(developerId, memberId, 'ADMIN', { startDate, endDate });
