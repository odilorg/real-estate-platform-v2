...
describe('AnalyticsService', () => {
  ...
  describe('getDashboard', () => {
    ...
    beforeEach(() => {
      ...
      prisma.agencyDeal.aggregate.mockResolvedValue({ _sum: { dealValue: 1000000 } });
      prisma.agencyCommission.aggregate.mockResolvedValue({
        _sum: { netAmount: 50000 },
        _count: 10,
      });
      prisma.agencyLead.groupBy.mockResolvedValue([
        { source: 'WEBSITE', _count: 40 },
        { source: 'REFERRAL', _count: 30 },
      ]);
      prisma.agencyDeal.groupBy.mockResolvedValue([
    ...
    it('should return dashboard analytics for admin', async () => {
      const result = await service.getDashboard(agencyId, memberId, 'ADMIN', {});
    ...
    it('should restrict agent to their own data', async () => {
      await service.getDashboard(agencyId, memberId, 'AGENT', {});

      // Verify that queries include memberId filtering
      expect(prisma.agencyLead.count).toHaveBeenCalledWith(
    ...
    it('should support different time periods', async () => {
      await service.getDashboard(agencyId, memberId, 'ADMIN', { period: 'WEEK' });
  ...
  describe('getLeadAnalytics', () => {
    ...
    it('should support different time periods', async () => {
      ...
      await service.getLeadAnalytics(agencyId, { period: 'QUARTER' });

      expect(prisma.agencyLead.groupBy).toHaveBeenCalledWith(
        expect.objectContaining({
          where: expect.objectContaining({
            createdAt: expect.objectContaining({
              gte: expect.any(Date),
  ...
  describe('getDealAnalytics', () => {
    ...
    it('should return deal analytics', async () => {
      ...
      prisma.agencyDeal.groupBy
        .mockResolvedValueOnce(mockStageDistribution)
        .mockResolvedValueOnce(mockStatusDistribution);

      const result = await service.getDealAnalytics(agencyId, {});
  ...
  describe('getAgentPerformance', () => {
    ...
    it('should return agent performance leaderboard', async () => {
      ...
      prisma.agencyDeal.count.mockResolvedValue(50);
      prisma.agencyDeal.aggregate.mockResolvedValue({ _sum: { dealValue: 1000000 } });
      prisma.agencyCommission.aggregate.mockResolvedValue({ _sum: { netAmount: 50000 } });

      const result = await service.getAgentPerformance(agencyId, {});
  ...
  describe('getRevenueAnalytics', () => {
    ...
    it('should return revenue analytics', async () => {
      ...
      const result = await service.getRevenueAnalytics(agencyId, {});
