...
export default function EditListingPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setTimeout(() => {
        router.push(`/agency/crm/listings/${params.id}`);
      }, 1500);
    } catch (error) {
  ...
  return (
    <div className="max-w-4xl mx-auto pb-24 md:pb-6">
      <div className="mb-6">
        <Link href={`/agency/crm/listings/${params.id}`}>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row gap-3 sticky bottom-0 bg-white p-4 border-t border-gray-200 -mx-4 md:mx-0 md:border-0 md:p-0 md:relative">
          <Link href={`/agency/crm/listings/${params.id}`} className="flex-1 sm:flex-initial">
            <button
              type="button"
