...
export default function MemberDetailPage({ params }: PageProps) {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      alert(t('alerts.deactivateError'));
    }
  };
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500">{t('detail.addedToTeam')} {formatDate(member.createdAt)}</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="bg-white p-6 rounded-lg shadow">
          <p className="text-sm text-gray-600">{t('stats.deals')}</p>
          <p className="text-2xl font-bold text-gray-900">{member._count.deals}</p>
        </div>
        <div className="bg-white p-6 rounded-lg shadow">
          <p className="text-sm text-gray-600">{t('stats.commissions')}</p>
          <p className="text-2xl font-bold text-gray-900">{member._count.commissions}</p>
        </div>
        <div className="bg-white p-6 rounded-lg shadow">
          <p className="text-sm text-gray-600">{t('stats.activities')}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          ...
          {(member.agentType || member.specializations || member.districts || member.languages) && (
            <div className="bg-white p-6 rounded-lg shadow">
              ...
              <div className="space-y-3">
                {member.agentType && (
                  <div>
                    <span className="text-sm text-gray-500">{t('detail.type')}:</span>
                    <p className="font-medium">{t(`agentTypes.${member.agentType}` as any, { defaultValue: member.agentType })}</p>
                  </div>
                )}
                {member.specializations && member.specializations.length > 0 && (
                  <div>
                    <span className="text-sm text-gray-500">{t('detail.specializations')}:</span>
                ...
                {member.licenseNumber && (
                  <div>
                    <span className="text-sm text-gray-500">{t('detail.licenseNumber')}:</span>
          ...
          {member.assignedLeads && member.assignedLeads.length > 0 && (
            <div className="bg-white p-6 rounded-lg shadow">
              <h2 className="text-xl font-semibold text-gray-900 mb-4">{t('detail.currentLeads')} ({member.assignedLeads.length})</h2>
              <div className="space-y-3">
        ...
        <div className="space-y-6">
          {/* Member Info */}
          <div className="bg-white p-6 rounded-lg shadow">
            <h2 className="text-xl font-semibold text-gray-900 mb-4">{t('sections.memberInfo')}</h2>
            <div className="space-y-3 text-sm">
              <div>
                <span className="text-gray-500">{t('detail.registrationDate')}:</span>
                <p className="font-medium">{formatDate(member.user.createdAt)}</p>
              </div>
              <div>
                <span className="text-gray-500">{t('detail.addedToTeam')}:</span>
                <p className="font-medium">{formatDate(member.createdAt)}</p>
              </div>
              <div>
                <span className="text-gray-500">{t('detail.role')}:</span>
                <p className="font-medium">{t(`roles.${member.role}` as any, { defaultValue: member.role })}</p>
              </div>
              <div>
