...
import { ChevronLeft, Save, Loader2, Calendar, Bell } from 'lucide-react';
...
export default function NewTaskPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccessMessage(t('notifications.created'));

      // Redirect after short delay
      setTimeout(() => {
        router.push(`/${locale}/developer/crm/tasks`);
      }, 1500);
    } catch (error) {
    ...
    <div className="min-h-screen bg-gray-50 p-4 sm:p-6 pb-28">
      ...
      <div className="max-w-3xl mx-auto">
        <div className="mb-6">
          ...
          <h1 className="text-2xl sm:text-3xl font-bold text-gray-900">{t('new.title')}</h1>
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow p-4 sm:p-6 space-y-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              >
                ...
                <option value="SEND_LISTINGS">{t('type.SEND_LISTINGS')}</option>
          ...
          <div>
            ...
            <select
              ...
              className="w-full px-4 py-3 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
            >
              <option value={0}>{t('reminder.none')}</option>
              <option value={15}>{t('reminder.15min')}</option>
              <option value={30}>{t('reminder.30min')}</option>
              <option value={60}>{t('reminder.1hour')}</option>
              <option value={180}>{t('reminder.3hours')}</option>
              <option value={1440}>{t('reminder.1day')}</option>
              <option value={4320}>{t('reminder.3days')}</option>
            </select>
          </div>
